...
# --- CREAR VARIABLES GLOBALES PARA EL SCRIPT DEL MODELO ---
data = preparar_dataset(DATA_DIR, DICT_PATH, image_size=config["image_size"])
...
ok_range = (labels.min() >= 0) and (labels.max() < num_classes)

total_archivos = len([f for f in os.listdir(DATA_DIR) if f.lower().endswith((".jpg",".jpeg",".png"))])
usadas = len(data.df)